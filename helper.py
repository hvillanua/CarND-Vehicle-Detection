...
def plot_pca_components(components):
    ...
    plt.plot(range(len(components)),components, marker='o')
    plt.show()
    
# Define a single function that can extract features using hog sub-sampling and make predictions
def find_cars(img, ystart, ystop, scale, svc, X_scaler, pca, cspace,
              orient, pix_per_cell, cell_per_block, spatial_size,
              hist_bins, verbose=False):
    ...
    # Scale the image to make windows bigger/smaller
    # scale > 1 big windows, scale < 1 small windows
    if scale != 1:
